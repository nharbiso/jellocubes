...
glm::vec<3, double> JelloCube::getStructuralForce(int i, int j, int k) {
    ...
    if(i > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-1, j, k), this->kElastic, this->restLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-1, j, k), this->dElastic);
    }
    if(i < this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i+1, j, k), this->kElastic, this->restLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i+1, j, k), this->dElastic);
    ...
    if(j > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j-1, k), this->kElastic, this->restLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j-1, k), this->dElastic);
    }
    if(j < this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j+1, k), this->kElastic, this->restLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j+1, k), this->dElastic);
    ...
    if(k > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j, k-1), this->kElastic, this->restLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j, k-1), this->dElastic);
    }
    if(k < this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j, k+1), this->kElastic, this->restLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j, k+1), this->dElastic);
...
glm::vec<3, double> JelloCube::getShearForce(int i, int j, int k) {
    ...
    if(j > 0 && k > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j-1, k-1), this->kElastic, axisDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j-1, k-1), this->dElastic);
    }
    if(j > 0 && k < this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j-1, k+1), this->kElastic, axisDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j-1, k+1), this->dElastic);
    ...
    if(i > 0 && k > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-1, j, k-1), this->kElastic, axisDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-1, j, k-1), this->dElastic);
    }
    if(i > 0 && k < this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-1, j, k+1), this->kElastic, axisDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-1, j, k+1), this->dElastic);
    ...
    if(i > 0 && j > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-1, j-1, k), this->kElastic, axisDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-1, j-1, k), this->dElastic);
    }
    if(i > 0 && j < this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-1, j+1, k), this->kElastic, axisDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-1, j+1, k), this->dElastic);
    ...
    if(i > 0 && j > 0 && k > 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-1, j-1, k-1), this->kElastic, cornerDiagRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-1, j-1, k-1), this->dElastic);
    }
...
glm::vec<3, double> JelloCube::getBendForce(int i, int j, int k) {
    ...
    if(i-2 >= 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i-2, j, k), this->kElastic, bendRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i-2, j, k), this->dElastic);
    }
    if(i+2 <= this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i+2, j, k), this->kElastic, bendRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i+2, j, k), this->dElastic);
    ...
    if(j-2 >= 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j-2, k), this->kElastic, bendRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j-2, k), this->dElastic);
    }
    if(j+2 <= this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j+2, k), this->kElastic, bendRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j+2, k), this->dElastic);
    ...
    if(k-2 >= 0) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j, k-2), this->kElastic, bendRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j, k-2), this->dElastic);
    }
    if(k+2 <= this->param) {
        force += this->hooksForce(getInd(i, j, k), getInd(i, j, k+2), this->kElastic, bendRestLen);
        force += this->dampeningForce(getInd(i, j, k), getInd(i, j, k+2), this->dElastic);
...
glm::vec<3, double> JelloCube::getCollisionForce(int i, int j, int k) {
    ...
    if(pos.x > bounds) {
        glm::vec<3, double> collisionPoint(bounds, pos.y, pos.z);
        double diff = pos.x - bounds;
        force += this->hooksForce(pos, collisionPoint, this->kCollision * diff, 0);
        force += this->dampeningForce(pos, collisionPoint, vel, boxVel, this->dCollision * diff);
    }
    if(pos.x < -bounds) {
        glm::vec<3, double> collisionPoint(-bounds, pos.y, pos.z);
        double diff = -(pos.x + bounds);
    ...
    if(pos.y > bounds) {
        glm::vec<3, double> collisionPoint(pos.x, bounds, pos.z);
        double diff = pos.y - bounds;
        force += this->hooksForce(pos, collisionPoint, this->kCollision * diff, 0);
        force += this->dampeningForce(pos, collisionPoint, vel, boxVel, this->dCollision * diff);
    }
    if(pos.y < -bounds) {
    ...
    if(pos.z > bounds) {
        glm::vec<3, double> collisionPoint(pos.x, pos.y, bounds);
        double diff = pos.z - bounds;
        force += this->hooksForce(pos, collisionPoint, this->kCollision * diff, 0);
        force += this->dampeningForce(pos, collisionPoint, vel, boxVel, this->dCollision * diff);
    }
    if(pos.z < -bounds) {
...
void JelloCube::computeAcceleration(std::vector<glm::vec<3, double>>& positions, std::vector<glm::vec<3, double>>& velocities, std::vector<glm::vec<3, double>>& acc) {
    for(int i = 0; i <= this->param; i++) {
        for(int j = 0; j <= this->param; j++) {
            for(int k = 0; k <= this->param; k++) {
                ...
                acc[ind] /= this->mass;
                acc[ind] += this->gravity;
            }
        }
    }
}

// Updates the positions and velocities of the jello cube's nodes using RK4 integration
void JelloCube::update() {
    ...
    //             this->nodes[ind] += this->dt * this->velocities[ind];
    //             this->velocities[ind] += this->dt * acc[ind];
    //         }
    //     }
    // }
    for(int i = 0; i <= this->param; i++) {
        for(int j = 0; j <= this->param; j++) {
            for(int k = 0; k <= this->param; k++) {
                int ind = getInd(i, j, k);
                F1pos[ind] = this->velocities[ind] * this->dt;
                F1vel[ind] = acc[ind] * this->dt;
    ...
    for(int i = 0; i <= this->param; i++) {
        for(int j = 0; j <= this->param; j++) {
            for(int k = 0; k <= this->param; k++) {
                int ind = getInd(i, j, k);
                F2pos[ind] = tmpVels[ind] * this->dt;
                F2vel[ind] = acc[ind] * this->dt;
    ...
    for(int i = 0; i <= this->param; i++) {
        for(int j = 0; j <= this->param; j++) {
            for(int k = 0; k <= this->param; k++) {
                int ind = getInd(i, j, k);
                F3pos[ind] = tmpVels[ind] * this->dt;
                F3vel[ind] = acc[ind] * this->dt;
    ...
    for(int i = 0; i <= this->param; i++) {
        for(int j = 0; j <= this->param; j++) {
            for(int k = 0; k <= this->param; k++) {
                int ind = getInd(i, j, k);
                F4pos[ind] = tmpVels[ind] * this->dt;
                F4vel[ind] = acc[ind] * this->dt;
